...
* https://docs.netapp.com/us-en/console-setup-admin/task-subscribe-standard-mode.html["了解如何訂閱NetApp資料服務（標準模式）"^]
* https://docs.netapp.com/us-en/console-setup-admin/task-subscribe-restricted-mode.html["了解如何訂閱NetApp資料服務（受限模式）"^]
...
NOTE: 對於 AWS，您的帳戶可能擁有多個 NetApp 市場訂閱。如果您僅關聯獨立式 Cloud Volumes ONTAP 訂閱，則可以使用 Essentials 部署 Cloud Volumes ONTAP，但設定備份與還原功能需要不同的訂閱。若要選擇用於 Cloud Volumes ONTAP 容量計費的訂閱（包括變更現有系統的計費方式），請前往 *Administration > Licenses and subscriptions > Billing preferences*。
...
. 從NetApp Console選單中，選擇「管理 >Licenses and subscriptions」。
. 選擇*市場訂閱*。
. 在要關聯的訂閱列中，開啟動作功能表，然後選取 *Associate* 。
. 在 *Associate the subscription* 對話方塊中、選取您要與訂閱建立關聯的 Console 組織。
. 選擇*關聯*。
...
. 從NetApp Console選單中，選擇「管理 >Licenses and subscriptions」。
. 選擇*市場訂閱*。
. 在包含要檢視其認證的訂閱的列中、選取 *View* 。如果多個認證與訂閱相關聯、則可能不會顯示任何認證、系統會引導您選取其他代理程式。
...
當您從雲端供應商（AWS、Google Cloud 或 Azure）取消訂閱時，Console 會將訂閱狀態顯示為 *Unsubscribed*。當您取消訂閱後，您也可以將其從 Licenses and subscriptions 中移除。

.開始之前
* 您只能移除狀態為 *Unsubscribed* 的訂閱。移除前，必須清除或重設所有相關設定、資料和配置。
+
如果訂閱仍處於設定狀態，則 * 移除 * 選項不可用。



.步驟
. 在NetApp Console選單中，選擇「管理 >Licenses and subscriptions」。
. 選擇*市場訂閱*。
. 在您要刪除的訂閱的行中，開啟操作選單並選擇*刪除*。
. 在 *Remove subscription* 對話方塊中，選取 *Remove* 以確認您要移除訂閱。

...
summary: 接受 Google Cloud 控制台中NetApp Console清單的 Marketplace 私人優惠。在控制台中設定 Google Cloud 憑證。 
---
= 接受並配置NetApp Console的 Google Cloud 專屬優惠
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
接受 Google Cloud 控制台中NetApp Console清單的 Marketplace 私人優惠。在控制台中設定 Google Cloud 憑證。
...
====
.步驟
. 選擇 NetApp 銷售團隊提供的專屬優惠啟動連結。該連結應類似於以下內容：
...
. 確認資訊準確無誤、接受條款，然後選取 **Accept** 。
. 建立訂閱時會出現一個載入圖示。
. 成功建立訂閱後，會彈出視窗顯示：*您的訂單要求已傳送至NetApp, Inc.*
. 選擇 **REGISTER WITH NETAPP, Inc.**。您將被重新導向至 NetApp Console 以完成訂閱設定。如果私人優惠已由貴組織中的其他人接受：
+
.. 在 Google Cloud Marketplace 中，前往**訂單**。
.. 找到狀態為**待定**的報價。
.. 選擇三點選單，然後選擇 **向 NetApp, Inc. 註冊**
...
+
.. 輸入「SaaS 市場訂閱」的顯示名稱。
.. 選擇應該有權存取訂閱的 Console *Organizations*。
.. 您也可以選擇啟用 **Replace existing subscription** 功能，以便在一個 Console 組織中自動替換現有的 Marketplace 訂閱。
+
[NOTE]
====
*** 此替換選項僅限於單一控制台帳戶。
